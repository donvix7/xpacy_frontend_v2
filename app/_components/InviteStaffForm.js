...
import { inviteStaffMember } from "../_lib/action";
import { FaAngleLeft, FaAngleRight, FaEnvelopeOpenText } from "react-icons/fa6";

const inviteSteps = [
    { step: 1, label: "Staff Info" },
    { step: 2, label: "Invitation" },
    { step: 3, label: "Review & Send" },
];

const defaultMessage = "You have been invited to join Xpacy as staff. Please sign up to get started.";
...
const InviteStaffForm = () => {
    ...
    const onSubmit = async (data) => {
        ...
        toast.promise(inviteStaffMember(formData), {
                    ...
                    <>
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="flex gap-4 items-center p-4 rounded-lg border border-primary-100 bg-primary-100/40 font-mono text-sm text-gray-700">
                                ...
                                <p>An invitation email will be sent to <span className="font-bold text-primary">{watched.email || "the staff member's email"}</span> with a link to join Xpacy.</p>
                            </div>
